...
#ident "@(#) LiS head.c 2.190 10/05/04 11:49:04 "
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
		   f->f_path.mnt, MNT_COUNT(f->f_path.mnt));
#else
...
#define RTN(e)  do { err=(e); goto return_point; } while (0)
	...
	if (LIS_DEBUG_WRITE)
	  printk(
	    "strwrite: stream %s: write length %lu less than min size of %ld\n",
			hd->sd_name, (unsigned long) ulen, hd->sd_minpsz) ;
...
#define	RTN(val)  do { err = val; goto return_point; } while (0)
	...
	case M_DATA:
	    ...
	    if (LIS_DEBUG_READ)
	      ...
	      "read %d error %d\n",
		   hd->sd_name, hd->sd_rdopt, (unsigned long) ulen, count, err) ;
