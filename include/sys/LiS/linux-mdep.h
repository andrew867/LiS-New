...
extern int lis_strflush(struct file *, fl_owner_t id);
#else
...
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
#define UID(fp)	  current->cred->uid
#define GID(fp)	  current->cred->gid
#define EUID(fp)  current->cred->euid
#define EGID(fp)  current->cred->egid
#else
...
#if defined(KERNEL_2_5)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
#define PGRP(fp)  pid_nr(task_pgrp(current))
#else
...
#ifdef CONFIG_SMP
#define	MNT_COUNT(m)  1	
//#define	MNT_COUNT(m)	atomic_read(&((m)->mnt_pcp->mnt_count))
#else
...
#endif
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,8,0)
#define FILE_MNT(f)	((f) ? (f)->f_path.mnt : (struct vfsmount *)NULL)
#else
