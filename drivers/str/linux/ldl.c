...
#else
STATIC int rcv_func(struct sk_buff *skb, struct ldldev *dev, 
		struct packet_type *pt, struct ldldev *orgdev);
#endif
...
#define	L	unsigned long
    ...
    printk("head=%lx data=%lx tail=%lx end=%lx truesize=%d\n"
	   ...
	   (L)skb->transport_header, (L)skb->network_header, (L)skb->mac_header, 
#else
...
STATIC INLINE void hangup_do(struct dl *dl)
{
	...
	if (dl->dlstate == DL_UNBOUND) {
		void ndev_release(struct dl *dl);
...
#endif
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
	fr_len = skb->tail - skb->mac_header;
#else
	...
	if ((dp = allocb(2 + fr_len, BPRI_LO)) != NULL) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
		memcpy(dp->b_wptr, skb->mac_header, fr_len);
#else
		...
		fr_ptr = &fr_buf[0];
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
		fr_len = lis_min(skb->end - skb->mac_header, LDL_MAX_HDR_LEN);
#else
		fr_len = lis_min(skb->end - skb->mac.raw, LDL_MAX_HDR_LEN);
#endif
		LISASSERT(fr_len > 0);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
		memcpy(fr_buf, skb->mac_header, fr_len);
#else
 ...
 */
STATIC void send_uderror(struct dl *dl, unsigned char *addr, int addrlen,
			 dl_ulong err, dl_ulong uerr)
{
	mblk_t * mp;
...
#endif
#endif
	if (dl->mkhdr(dl, (unsigned char *)reqp + reqp->dl_dest_addr_offset,
		      dlen, skb) != 1) {
#ifdef KERNEL_2_1
		kfree_skb(skb);
#else
...
#ifdef KERNEL_2_1
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32)  /* TETHER */
	skb->network_header = skb->data + dl->machdr_len;
#else
